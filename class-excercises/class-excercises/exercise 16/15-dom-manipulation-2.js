...
newEl.setAttribute("id", "newText");

let oldChild = document.querySelector("#newText");

let parentElem = document.getElementById("insert-p");

let newElementText = document.createElement("p");

let textReplacement = (newElementText.innerText = "This is a new comment");

parentElem.replaceChild(newElementText, oldChild);
...
// PREGUNTA --> EL ORDEN DEL STYLE MATTERS!!! WHY? CHECK CSS

const buttonElemToggle = document.getElementById("newButton");

const toggleButtonFunction = () => {
  document.querySelector(".red-box").classList.toggle("green-box");
};

buttonElemToggle.addEventListener("click", toggleButtonFunction);
...
newDiv.className = "green-box";
console.log(newDiv);

let addNewElement = document
  .querySelector("#parent")
  .appendChild(newDiv);
...
const newButtonCool = document.createElement("button");
let addNewButton = document.querySelector("#parent").appendChild(newButton);
let textButton = document.createTextNode("Texto insertado")
let 

newButton.addEventListener("click", FunctionButton);
